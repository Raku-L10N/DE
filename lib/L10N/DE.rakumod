...
#- Generated on 2025-06-27T18:27:35+02:00 by update-localization.raku
...
role L10N::DE {
    ...
    token enum-Broken { Gebrochen}
    token enum-False { Falsch}
    token enum-FileChanged { FileChanged}
    token enum-FileRenamed { FileRenamed}
    token enum-Kept { Gehalten}
    token enum-Less { Weniger}
    token enum-LittleEndian { LittleEndian}
    token enum-More { Mehr}
    token enum-NativeEndian { NativeEndian}
    token enum-Planned { Versprochen}
    token enum-Same { Dasselbe}
    ...
    method trait-is2ast {
        my constant %mapping = "kopie", "copy", "standard", "default", "VERALTET", "DEPRECATED", "äquivalent", "equiv", "implementierungsdetail", "implementation-detail", "lockerer", "looser", "rein", "pure", "roh", "raw", "lesen-schreiben", "rw", "enger", "tighter";
        my $ast := self.ast;
        my $name := $ast ?? $ast.simple-identifier !! self.Str;
        if %mapping{$name} -> $original {
            RakuAST::Name.from-identifier($original)
    ...
    method adverb-rx2str (str $key) {
        my constant %mapping = "fortsetzen", "continue", "er", "ex", "erschöpfend", "exhaustive", "te", "nd", "te", "nth", "üb", "ov", "überlappung", "overlap", "ratsche", "ratchet", "te", "rd", "te", "st", "bis", "to", "mal", "x";
        %mapping{$key} // $key
    }
